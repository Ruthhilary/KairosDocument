...
This guide walks an Admin through setting up Kairos for a new church from scratch. If your church is already set up, skip to [Platform](/platform/dashboard).

## Step 1 — Create a Region

Regions are geographic groupings for your branches. Start here.

Go to **Regions** in the sidebar and click **+ New Region**. Enter the region name and country.
...
You can add members one at a time via **Members → + Add Member**, or bulk import them via **Import CSV**. Members need to be assigned to a branch during creation.

After adding members, any who self-registered will appear in the **Approval Queue** under the Members page header. Approve them to grant access.
